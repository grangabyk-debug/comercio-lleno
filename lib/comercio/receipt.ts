...
function receiptPdfBytes(sale:Sale,company:CompanyProfile){
  ...
  if(fiscal)lines.push({text:`CAE: ${sale.cae||'—'}`},{text:`Vencimiento CAE: ${sale.caeExpiration||'—'}`},{text:qrUrl?'QR fiscal ARCA: escanear para verificar':'QR fiscal ARCA no disponible'})
