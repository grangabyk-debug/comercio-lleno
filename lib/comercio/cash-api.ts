import type { CashRegister,TenantSession } from './types'
...
export type CashSummary={sales_total:number;sales_count:number;payments:Record<string,number>;cash_sales:number;income:number;expenses:number;egress:number;expected_cash:number;counted_cash?:number;difference?:number;opening_amount:number;opened_at:string;closed_at:string}
export type CashHistory={id:string;cash_register_id?:string|null;opened_at:string;closed_at:string;opening_amount:number;closing_amount:number;summary:CashSummary}

async function request<T>(session:TenantSession,path:string,init:RequestInit={}){const r=await fetch(`${URL}/rest/v1/${path}`,{...init,headers:{apikey:KEY,Authorization:`Bearer ${session.token}`,'Content-Type':'application/json',...(init.headers||{})},cache:'no-store'});const d=await r.json().catch(()=>null);if(!r.ok)throw new Error(d?.message||d?.error||`Error de caja (${r.status})`);return d as T}
async function notifyCashClose(session:TenantSession,historyId:string){try{const response=await fetch('/api/redesign/whatsapp/automation',{method:'POST',headers:{Authorization:`Bearer ${session.token}`,'Content-Type':'application/json'},body:JSON.stringify({action:'cash_close',historyId}),cache:'no-store'});return await response.json().catch(()=>({ok:false}))}catch{return{ok:false}}}
export async function openCashSecure(session:TenantSession,amount:number){return request<CashRegister&{close_summary?:CashSummary}>(session,'rpc/open_cash_register_authorized',{method:'POST',body:JSON.stringify({p_opening_amount:Math.max(0,amount)})})}
