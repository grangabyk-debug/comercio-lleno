...
export function createStaticQrOrder(session:TenantSession,amount:number,externalReference:string){const idempotency=typeof crypto!=='undefined'&&crypto.randomUUID?crypto.randomUUID():`qr-${Date.now()}`;return callStaticQr<{ok:true;order:StaticQrOrder}>(session,{action:'create_order',amount,external_reference:externalReference,idempotency_key:idempotency})}
...
export function cancelStaticQrOrder(session:TenantSession,orderId:string){return callStaticQr<{ok:true;order:StaticQrOrder}>(session,{action:'cancel_order',order_id:orderId})}
