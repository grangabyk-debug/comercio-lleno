...
export const PAYMENT_METHODS = ['Efectivo', 'Débito', 'Crédito', 'Transferencia', 'Mercado Pago', 'Mercado Pago QR', 'Billetera Virtual'] as const

export function normalizePaymentParts(parts: PaymentPart[] | null | undefined, total: number): PaymentPart[] {
  const safeTotal = Math.max(0, Number(total || 0))
  const clean = (parts || []).map(part => ({ method: String(part?.method || '').trim(), amount: Math.max(0, Number(part?.amount || 0)) })).filter(part => part.method && part.amount > 0)
...
export function paymentPartsForSale(sale: Sale): PaymentPart[] {
  ...
  if (Array.isArray(raw)) { const normalized = normalizePaymentParts(raw as PaymentPart[], sale.total); if (normalized.length) return normalized }
...
export function paymentAmountForSale(sale: Sale, method: string | RegExp) {
  return paymentPartsForSale(sale).reduce((sum, part) => {
    const matches = typeof method === 'string' ? part.method.toLowerCase() === method.toLowerCase() : method.test(part.method)
...
export function paymentTotalsByMethod(sales: Sale[]) {
  ...
  for (const sale of sales) {
    ...
    for (const part of paymentPartsForSale(sale)) {
      ...
      if (!seen.has(part.method)) { current.operations += 1; seen.add(part.method) }
