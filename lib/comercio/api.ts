...
const ARCA_INVOICE_ENDPOINT = `${SUPABASE_URL}/functions/v1/arca-invoice`
...
export async function loadCommerceSnapshot(session: TenantSession): Promise<CommerceSnapshot> {
  ...
  return {
    ...
    products: (products || []).map((p) => ({ ...p, price: Number(p.price || 0), stock: Number(p.stock || 0), cost: p.cost == null ? null : Number(p.cost) })),
    sales: (saleRows || []).map(mapSale),
    customers: customers || [],
    cashRegister: cashRows[0] ? { ...cashRows[0], opening_amount: Number(cashRows[0].opening_amount || 0) } : null,
    cashMovements: (cashMovements || []).map((m) => ({ ...m, amount: Number(m.amount || 0) })),
  }
}

export async function authorizeFiscalInvoice(session: TenantSession, amount: number, requestId: string): Promise<FiscalInvoice> {
  const response = await fetch(ARCA_INVOICE_ENDPOINT, {
    method: 'POST',
    headers: {
      apikey: PUBLISHABLE_KEY,
      Authorization: `Bearer ${session.token}`,
  ...
  if (!response.ok || !data?.ok || !data?.invoice?.cae) {
    const errors = Array.isArray(data?.invoice?.errors) ? data.invoice.errors.map((e: any) => `${e.code}: ${e.msg}`).join(' | ') : ''
    throw new Error(data?.error || errors || `ARCA respondió ${response.status}`)
  }
...
export async function persistAuthorizedSale(
  ...
  await Promise.all(stockUpdates.map((item) => rest(session, `products?id=eq.${encodeURIComponent(item.id)}&company_id=eq.${encodeURIComponent(session.companyId)}`, {
    method: 'PATCH',
