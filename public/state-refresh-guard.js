...
function check(){
 ...
 var stale=false;
 if(isHome&&todayTotal>0){var cards=document.querySelectorAll('.cards .card');if(cards[0]){var strong=cards[0].querySelector('strong');var txt=(strong&&strong.textContent||'').replace(/[^0-9]/g,'');if(!txt||Number(txt)===0)stale=true}}
...
setTimeout(check,700);setInterval(check,1500);window.addEventListener('storage',check);
})();