...
export default function MobileAiAssistant(){
  ...
  const[pos,setPos]=useState<Pos>({x:12,y:0})
  const drag=useRef<{id:number;startX:number;startY:number;originX:number;originY:number;moved:boolean}|null>(null)
  useEffect(()=>{let cancelled=false;const sync=async()=>{const s=readTenantSession();if(!s){if(!cancelled)setAvailable(false);return}let cfg=readCachedMobileSettings(s.companyId);try{cfg=await loadMobileSettings(s)}catch{}if(!cancelled)setAvailable(cfg.aiEnabled)};void sync();const changed=()=>void sync();window.addEventListener('comercio:mobile-settings',changed);setPos({x:12,y:Math.max(90,window.innerHeight-180)});return()=>{cancelled=true;window.removeEventListener('comercio:mobile-settings',changed)}},[])
