...
export default function MobileAiAssistant(){
  ...
  useEffect(()=>{const sync=()=>setAvailable(Boolean(readTenantSession()));sync();const timer=window.setInterval(sync,1000);setPos({x:12,y:Math.max(90,window.innerHeight-180)});return()=>window.clearInterval(timer)},[])
