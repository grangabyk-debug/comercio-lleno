...
function employmentWarningFor(draft:Draft){
 const hay=norm(`${draft.title} ${draft.category} ${draft.description} ${draft.duration_text} ${draft.compensation_text}`)
 if(!hay)return''
 ...
 if(hard.test(hay)||strong.test(hay)||(role.test(hay)&&hiring.test(hay)))return'Esto parece una búsqueda laboral. Publicalo desde una cuenta de Empresa en Empleos. Servicio Flex es para servicios puntuales e independientes, no para cubrir puestos ni relaciones laborales.'
 return''
}
...
export default function ChangasExplorer({gigs,categories}:{gigs:PreviewGig[];categories:string[]}){
 ...
 const[timingFilter,setTimingFilter]=useState('Todos')
 ...
 useEffect(()=>{
  if(!publish&&!authChoice&&!open)return
  const previous=document.body.style.overflow;document.body.style.overflow='hidden'
  return()=>{document.body.style.overflow=previous}
 },[publish,authChoice,open])

 const all=useMemo<FlexCard[]>(()=>[...real,...gigs.map(g=>({...g,publisherKind:'person'} as FlexCard))],[real,gigs])
 ...
 const filtered=useMemo(()=>{
  ...
  return all.filter(g=>{
   const hay=norm(`${g.title} ${g.summary} ${g.poster} ${g.category} ${g.tags.join(' ')} ${g.location}`)
   ...
   const timingOk=timingFilter==='Todos'||(timingFilter==='Hoy'?/hoy/i.test(g.when):timingFilter==='A coordinar'?/coordinar/i.test(g.when):timingFilter==='Con fecha'?!/coordinar/i.test(g.when):true)
   return(!q||hay.includes(q))&&catOk&&zoneOk&&publisherOk&&timingOk
  })
 },[all,query,zone,category,publisherFilter,timingFilter])
 const visible=useMemo(()=>filtered.slice(0,visibleCount),[filtered,visibleCount])
 useEffect(()=>{setVisibleCount(PAGE_SIZE)},[query,zone,category,publisherFilter,timingFilter])
 ...
 const employmentWarning=useMemo(()=>employmentWarningFor(draft),[draft.title,draft.category,draft.description,draft.duration_text,draft.compensation_text])
 ...
 return <>
  ...
  <div className="pm38-flex-search"><label><span>Servicio o palabra clave</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Ej. muebles, fotografía, Excel"/></label><label><span>Zona o modalidad</span><input value={zone} onChange={e=>setZone(e.target.value)} placeholder="Ej. Palermo, Córdoba, remoto"/></label><div><span>RESULTADOS</span><b>{filtered.length}</b><small>servicios encontrados</small></div></div>
