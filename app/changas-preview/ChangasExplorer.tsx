...
export default function ChangasExplorer({gigs,categories}:{gigs:PreviewGig[];categories:string[]}){
 ...
 return <>
  ...
  {publish&&<div className="pm34-publish-overlay" role="dialog" aria-modal="true" aria-labelledby="pm34-publish-title"><form className="pm34-publish-card" onSubmit={create}>
   ...
   <header className="pm34-publish-head"><div><span className="pm34-kicker">PUBLICAR SERVICIO FLEX</span><h2 id="pm34-publish-title">Creá una publicación clara en pocos minutos.</h2><p>Completá lo esencial para que la otra persona entienda qué necesitás, dónde, cuándo y cuál es el importe estimado antes de escribirte.</p></div><div className="pm34-publish-steps" aria-hidden="true"><span><i>1</i> Servicio</span><span><i>2</i> Fecha e importe</span><span><i>3</i> Publicación</span></div></header>
   ...
   <aside className="pm34-publish-preview"><div className="pm34-preview-sticky"><span className="pm34-preview-label">VISTA PREVIA</span><div className="pm34-preview-photo" style={{backgroundImage:`url(${previewImage})`}}><span>{previewWhen}</span></div><div className="pm34-preview-body"><div className="pm34-preview-top"><span>{draft.category||'Categoría'}</span><b>{draft.compensation_text||'Importe a definir'}</b></div><h3>{draft.title||'Tu servicio va a verse así'}</h3><p>{draft.description||'Mientras completás el formulario, vas a ver acá un resumen de lo que recibirán las personas interesadas.'}</p><div className="pm34-preview-meta"><span>⌖ {draft.location_text||'Zona a definir'}</span><span>◷ {draft.duration_text||'Duración a coordinar'}</span></div></div></div><div className="pm34-preview-help"><b>Antes de publicar</b><span>✓ Tarea concreta</span><span>✓ Importe visible</span><span>✓ Zona o modalidad</span><span>✓ Fecha si ya la sabés</span><span>✓ Sin datos sensibles</span></div></aside></div>
