...
import {AiSignature,ArgentinaPresence,HumanSupportVisual,WhatsAppPhoneVisual,SecuritySeals,GooglePresence} from './LandingEnhancements'
...
const barberPhoto='https://images.pexels.com/photos/7697320/pexels-photo-7697320.jpeg?auto=compress&cs=tinysrgb&w=1600'
...
const faq=[
  ...
  ['¿Puedo probarlo antes de pagar?','Sí. Empezás con 14 días gratis. Después pagás $14.900 por mes durante los primeros 3 meses y, desde el cuarto mes, el valor habitual es $39.900 por mes.'],
]
...
export default function LandingPage(){
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.editorialSection} id="producto"><div className={styles.editorialHeading}><p>TODO LO QUE PASA EN EL MOSTRADOR</p><h2>Un sistema que se siente rápido porque está diseñado alrededor de la operación.</h2></div><div className={styles.editorialGrid}><article className={`${styles.mediaCard} ${styles.mediaCardLarge}`}><img src={bakeryPhoto} alt="Comerciante atendiendo a un cliente en un local"/><div><span>VENTAS Y CAJA</span><h3>Cobrá sin frenar la fila.</h3><p>Buscá o escaneá productos, elegí el medio de pago, calculá vuelto y dejá cada operación registrada.</p></div></article><article className={styles.lightCard}><span>FACTURACIÓN ELECTRÓNICA</span><h3>ARCA integrado al flujo de venta.</h3><p>Una vez configurado el comercio, facturación y venta dejan de ser dos procesos separados.</p><div className={styles.arcaWord}>ARCA</div></article><article className={styles.darkFeature}><span>PEDIDO IA+</span><h3>Reposición basada en lo que realmente se mueve.</h3><p>Demanda reciente, stock actual y señales del día a día ayudan a priorizar qué comprar primero.</p><div style={{margin:'18px 0 28px'}}><AiSignature compact/></div><div className={styles.orderList}>{['Detergente concentrado','Alcohol etílico','Desengrasante','Jabón líquido'].map((x,i)=><div key={x}><b>{String(i+1).padStart(2,'0')}</b><span>{x}</span></div>)}</div></article><article className={styles.mediaCard}><img src={inventoryPhoto} alt="Persona revisando inventario en un comercio"/><div><span>INVENTARIO</span><h3>Controlá stock donde está el producto.</h3><p>La versión móvil permite consultar y actualizar sin volver a la computadora.</p></div></article><article className={styles.simpleFeature}><SimpleMode/></article></div></section>

    <section className={styles.argentinaSection} id="comercios"><div className={styles.argentinaCopy}><p>UNA RED QUE SIGUE CRECIENDO</p><h2>Más de 150 comercios ya trabajan con Comercio Lleno en Argentina.</h2><p>Almacenes, locales de cercanía, perfumerías, bazares, servicios y otros negocios usan la plataforma para ordenar ventas, stock y caja desde una misma cuenta.</p><div className={styles.metricRow}><div><strong>150+</strong><span>comercios</span></div><div><strong>24/7</strong><span>acceso web</span></div><div><strong>1 sistema</strong><span>para toda la operación</span></div></div></div><ArgentinaPresence mapSrc={argentinaMap}/></section>

    <section className={styles.humanMosaic}><div className={styles.humanIntro}><p>HECHO PARA NEGOCIOS REALES</p><h2>El mismo sistema puede acompañar formas muy distintas de trabajar.</h2></div><div className={styles.mosaicGrid}><figure><img src={barberPhoto} alt="Profesional atendiendo en una barbería"/><figcaption><b>Servicios</b><span>Clientes, productos, ventas y caja.</span></figcaption></figure><figure><img src={furniturePhoto} alt="Personas trabajando en un showroom"/><figcaption><b>Showrooms y locales</b><span>Catálogo, stock, precios y seguimiento.</span></figcaption></figure><figure><img src={bakeryPhoto} alt="Local minorista atendiendo clientes"/><figcaption><b>Comercios de cercanía</b><span>Velocidad de mostrador y facturación.</span></figcaption></figure></div></section>
    ...
    <section className={styles.priceSection} id="precio"><div className={styles.priceCopy}><p>OPORTUNIDAD ESPECIAL</p><h2>Probalo con tu negocio funcionando.</h2><p>Cargá productos, hacé ventas y decidí después de usarlo de verdad. Empezás con 14 días gratis antes del primer cobro.</p></div><div className={styles.priceCard}><span>14 DÍAS GRATIS</span><del style={{display:'block',fontSize:20,color:'#9aa39f',marginTop:8}}>$39.900 / mes</del><strong>$14.900 <small>/ mes x 3 meses</small></strong><p>Ahorrás $25.000 por mes durante tus primeros 3 meses. Desde el cuarto mes, el valor habitual es $39.900/mes.</p><Link href="/prueba-gratis">Crear mi cuenta</Link><small>El módulo WhatsApp + IA se contrata por separado.</small></div></section>
