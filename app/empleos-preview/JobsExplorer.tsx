...
function jobDate(job:PreviewJob){const raw=String(job.checkedAt||'').trim();if(!raw)return null;const date=/^\d{4}-\d{2}-\d{2}$/.test(raw)?new Date(`${raw}T12:00:00`):new Date(raw);return Number.isNaN(date.getTime())?null:date}
function jobDateValue(job:PreviewJob){return jobDate(job)?.getTime()||0}
function jobDateLabel(job:PreviewJob){const date=jobDate(job);if(!date)return'';const now=new Date();const today=new Date(now.getFullYear(),now.getMonth(),now.getDate()).getTime();const day=new Date(date.getFullYear(),date.getMonth(),date.getDate()).getTime();const diff=Math.round((today-day)/86400000);if(diff===0)return'Hoy';if(diff===1)return'Ayer';return date.toLocaleDateString('es-AR',{day:'2-digit',month:'2-digit',year:'2-digit'})}
function jobDateTitle(job:PreviewJob){const date=jobDate(job);return date?`Fecha de publicación o última actualización: ${date.toLocaleDateString('es-AR',{day:'2-digit',month:'2-digit',year:'numeric'})}`:''}

export default function JobsExplorer({jobs}:{jobs:PreviewJob[]}){
  const searchParams=useSearchParams()
  const [query,setQuery]=useState(''),[location,setLocation]=useState(''),[mode,setMode]=useState('Todos'),[area,setArea]=useState('Todas'),[schedule,setSchedule]=useState('Todos'),[province,setProvince]=useState('Todas'),[city,setCity]=useState('Todas'),[dateOrder,setDateOrder]=useState<DateOrder>('newest')
  ...
  const filtered=useMemo(()=>{
    ...
    if(profileOrder&&profile)return [...list].sort((a,b)=>matchScore(b,profile)-matchScore(a,profile)||jobDateValue(b)-jobDateValue(a)||a.title.localeCompare(b.title))
    if(dateOrder==='newest')return [...list].sort((a,b)=>jobDateValue(b)-jobDateValue(a)||a.title.localeCompare(b.title))
    if(dateOrder==='oldest')return [...list].sort((a,b)=>jobDateValue(a)-jobDateValue(b)||a.title.localeCompare(b.title))
    return list
  ...
  return <div className="pm-jobs">
    ...
    <div className="pm-workspace">
      ...
      <section className="pm-results" aria-live="polite"><div className="pm-results-head"><div><span>OPORTUNIDADES</span><strong>{filtered.length} resultados</strong></div><small>{profileOrder&&profile?'Ordenados por afinidad con tu perfil':dateOrder==='newest'?'Más recientes primero':dateOrder==='oldest'?'Más antiguos primero':'Buenos Aires primero · luego Argentina'}</small></div><div className="pm-job-list">{filtered.length?filtered.map(job=>{const visual=jobVisual(job),score=profile?matchScore(job,profile):0,dateLabel=jobDateLabel(job);return <article key={job.slug} className="pm-job-card" data-selected={selected?.slug===job.slug} onClick={()=>setSelectedSlug(job.slug)}>
