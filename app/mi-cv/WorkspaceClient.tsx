...
export default function WorkspaceClient(){
 ...
 async function load(){
   ...
   try{
     ...
     if(pending){setPhoto(pending);setPhotoNote(pendingSource==='auto'?'✓ Conservamos automáticamente la foto que encontramos en tu CV.':'✓ Foto optimizada y aplicada en este dispositivo.')}
 ...
 function changeTab(next:'cv'|'linkedin'|'carta'|'entrevista'){setTab(next);void trackCvEvent('workspace_tab_view',{tab:next},'/mi-cv');window.scrollTo({top:Math.max(0,document.querySelector('[data-workspace-tabs]')?.getBoundingClientRect().top||0)+window.scrollY-90,behavior:'smooth'})}
 ...
 return <>
  ...
  <div className={styles.grid}><div className={styles.workspaceMain}>
   <div className={styles.workspaceGuide}><b>Recorré tu candidatura completa</b><span>Empezá por el CV final y después revisá LinkedIn, el mensaje de postulación y la entrevista. Podés volver a cualquiera cuando quieras.</span></div>
   <div className={styles.tabs} data-workspace-tabs><button data-tab="cv" data-on={tab==='cv'} onClick={()=>changeTab('cv')}>1 · CV final</button><button data-tab="linkedin" data-on={tab==='linkedin'} onClick={()=>changeTab('linkedin')}>2 · LinkedIn</button><button data-tab="carta" data-on={tab==='carta'} onClick={()=>changeTab('carta')}>3 · Postulación</button><button data-tab="entrevista" data-on={tab==='entrevista'} onClick={()=>changeTab('entrevista')}>4 · Entrevista</button></div>
   {tab==='cv'&&<div className={styles.resumeWrap}><div className={`${styles.resume} ${template==='modern'?styles.modern:styles.classic} ${paletteClass}`}>
