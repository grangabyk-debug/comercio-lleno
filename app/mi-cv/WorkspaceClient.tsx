...
const OWNER_TEST_API='https://pejkycdttogpmmdntzuq.supabase.co/functions/v1/cv-ai-owner-test'
...
async function ownerTestCall(body:any){const r=await fetch(OWNER_TEST_API,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const d=await r.json().catch(()=>({ok:false,error:'Respuesta inválida'}));if(!r.ok||!d?.ok)throw new Error(d?.error||'No pudimos validar el modo propietario.');return d}
...
export default function WorkspaceClient(){
 ...
 const [ownerTest,setOwnerTest]=useState(false)
 ...
 async function load(){
   ...
   try{
     ...
     let secureOwnerTest=false
     if(useToken){
       try{
         const test=await ownerTestCall({action:'status',session_token:useToken})
         secureOwnerTest=!!test.owner_test_mode
         setOwnerTest(secureOwnerTest)
         if(secureOwnerTest){
           localStorage.setItem('cv_owner_test_mode','1')
           if(test.plan!=='active')await ownerTestCall({action:'set_plan',session_token:useToken,plan:'active'})
         }
       }catch{
         setOwnerTest(false)
       }
     ...
     const accountOwner=data.account?.role==='owner'
     const isOwner=accountOwner||secureOwnerTest
     setOwner(isOwner)
