...
type Palette='navy'|'graphite'|'violet'
...
const paletteHex:Record<Palette,{accent:string;side:string}>={navy:{accent:'#1f4b73',side:'#172532'},graphite:{accent:'#46545b',side:'#202427'},violet:{accent:'#6957ff',side:'#1d1a2b'}}
...
export default function WorkspaceClient(){
 ...
 const highlightedSkills=useMemo(()=>resume?.skills.slice(0,8)||[],[resume])
 ...
 async function load(){
   ...
   try{
     ...
     const storedPalette=(localStorage.getItem('postula_cv_palette')||'navy') as Palette;if(['navy','graphite','violet'].includes(storedPalette))setPalette(storedPalette)
     if(!data.resume&&((data.plan==='pro'||data.plan==='active')||isOwner)){
       ...
       setQuality(Number(data.quality?.confidence||100));setPhoto(data.photo_url||null);void trackCvEvent('cv_pro_generated',{plan:isOwner?'owner':data.plan||'pro'},'/mi-cv')
     }
 ...
 async function uploadPhoto(file:File){setError('');try{if(file.size>1048576)throw new Error('La foto debe pesar menos de 1 MB.');const data=await callPro({action:'save_photo',token,mime:file.type,data:await toBase64(file)});setPhoto(data.photo_url);setMessage('Foto guardada de forma privada y aplicada al CV.');void trackCvEvent('photo_uploaded',{file_type:file.type},'/mi-cv')}catch(e){setError(e instanceof Error?e.message:'No pudimos guardar la foto.')}}
 function printCv(){setTab('cv');void trackCvEvent('cv_pdf_downloaded',{},'/mi-cv');window.setTimeout(()=>window.print(),100)}
 function downloadWord(){if(!resume)return;void trackCvEvent('cv_word_downloaded',{},'/mi-cv');const jobs=resume.experience.map(j=>`<h3>${esc(j.role)} · ${esc(j.company)}</h3><small>${esc([j.start_date,j.end_date].filter(Boolean).join(' — '))}</small><ul>${j.bullets.map(b=>`<li>${esc(b)}</li>`).join('')}</ul>`).join('');const theme=paletteHex[palette];const html=`<html><head><meta charset="utf-8"><style>body{font-family:Arial;color:#17191d;max-width:760px;margin:35px auto}h1{font-size:30px}h2{font-size:14px;border-bottom:1px solid #bbb}p,li{font-size:11px;line-height:1.5}.accent{color:${theme.accent}}</style></head><body><h1>${esc(resume.candidate_name)}</h1><p class="accent"><b>${esc(resume.headline)}</b></p><p>${esc(contact)}</p><h2>Perfil profesional</h2><p>${esc(resume.summary)}</p><h2>Experiencia</h2>${jobs}<h2>Formación</h2>${resume.education.map(e=>`<p><b>${esc(e.degree)}</b> · ${esc(e.institution)} ${e.date?esc(e.date):''}</p>`).join('')}<h2>Habilidades destacadas</h2><p>${highlightedSkills.map(esc).join(' · ')}</p></body></html>`;const blob=new Blob([html],{type:'application/msword;charset=utf-8'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`CV-${resume.candidate_name.replace(/[^a-z0-9]+/gi,'-')}.doc`;a.click();URL.revokeObjectURL(url)}
 async function copy(text:string){try{await navigator.clipboard.writeText(text);setMessage('Copiado al portapapeles.')}catch{}}
 function changeTab(next:'cv'|'linkedin'|'carta'|'entrevista'){setTab(next);void trackCvEvent('workspace_tab_view',{tab:next},'/mi-cv')}
 ...
 if(loading)return <div className={styles.loading}><div><div className={styles.spinner}/><h2>Preparando tu espacio…</h2><p>Estamos armando el CV y verificando que la redacción siga siendo fiel a tu experiencia.</p></div></div>
 ...
 const paletteClass=palette==='graphite'?styles.paletteGraphite:palette==='violet'?styles.paletteViolet:styles.paletteNavy
 ...
 return <>
  <section className={styles.hero}><div><span>CV PRO</span><h1>Tu candidatura ya está lista.</h1><p>Revisala, elegí diseño y color, y guardala. La redacción fue comparada contra los hechos de tu CV antes de entregarse.</p></div>{quality!==null&&<div className={styles.quality}>✓ Control factual aprobado · {quality}%</div>}</section>
  ...
  <div className={styles.grid}><div><div className={styles.resumeWrap}><div className={`${styles.resume} ${template==='modern'?styles.modern:styles.classic} ${paletteClass}`}>
   ...
   <div className={styles.tabs}><button data-tab="cv" data-on={tab==='cv'} onClick={()=>changeTab('cv')}>CV final</button><button data-tab="linkedin" data-on={tab==='linkedin'} onClick={()=>changeTab('linkedin')}>LinkedIn</button><button data-tab="carta" data-on={tab==='carta'} onClick={()=>changeTab('carta')}>Postulación</button><button data-tab="entrevista" data-on={tab==='entrevista'} onClick={()=>changeTab('entrevista')}>Entrevista</button></div>
