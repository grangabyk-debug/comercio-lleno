...
import { CV_ACCOUNT_API, CV_API, CV_PRO_API, SESSION_KEY, authHeaders, cvAuthClient, postCv } from '../cv-ia/cvAuth'
...
export default function ActiveClient(){
 ...
 async function submit(e:FormEvent){e.preventDefault();if(busy)return;setBusy(true);setError('');setMessage('');try{const d=await pro({action:'active_adapt',token,company_name:company,role_name:role,job_text:job});setMessage('Candidatura preparada y validada. La agregamos al tablero.');setCompany('');setRole('');setJob('');const list=await pro({action:'applications',token});setApps(list.applications||[]);setRemaining(Number(list.remaining||0));const created=(list.applications||[]).find((a:AppItem)=>a.id===d.application_id);if(created)setSelected(created)}catch(e){setError(e instanceof Error?e.message:'No pudimos preparar esta candidatura.')}finally{setBusy(false)}}
 async function move(app:AppItem,status:string){try{await pro({action:'update_application',token,id:app.id,status});setApps(x=>x.map(a=>a.id===app.id?{...a,status}:a));if(selected?.id===app.id)setSelected({...app,status})}catch(e){setError(e instanceof Error?e.message:'No pudimos actualizar el estado.')}}
