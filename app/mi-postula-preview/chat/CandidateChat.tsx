...
export default function CandidateChat(){
 ...
 async function answerInterview(interview:Interview,status:'accepted'|'declined'){if(busy)return;setBusy(true);setNotice('');try{const authHeaders=await authHeader();if(!('Authorization' in authHeaders))return;const r=await fetch('/api/postula/messages',{method:'POST',headers:{'Content-Type':'application/json',...authHeaders},body:JSON.stringify({action:'respond_interview',audience:'candidate',conversation_id:active,interview_id:interview.id,status})});const d=await r.json().catch(()=>({}));if(!r.ok||!d?.ok)throw new Error(d?.error||'No pudimos responder la entrevista.');setInterviews(v=>v.map(x=>x.id===interview.id?d.interview:x));if(d.message)setMessages(v=>[...v,d.message]);setNotice(status==='accepted'?'Entrevista confirmada. La empresa recibió tu respuesta y ya quedó agendada en tu calendario.':'Avisamos a la empresa que no podés aceptar esa propuesta. Podés escribir para coordinar otra opción.');void loadThreads(active)}catch(e){setNotice(e instanceof Error?e.message:'No pudimos responder.')}finally{setBusy(false)}}
 ...
 return <main className="pmc-page">
  ...
  <section className="pmc-chat">
   ...
   {thread.conversation_kind!=='flex'&&interviews.length>0&&<div className="pmc-interviews">{interviews.slice().reverse().map(iv=><article key={iv.id} data-status={iv.status}><div><span>ENTREVISTA</span><b>{interviewWhen(iv.scheduled_for)}</b><p>{interviewMode(iv.mode)} · {iv.duration_minutes} min{iv.location_text?` · ${iv.location_text}`:''}</p>{iv.notes&&<small>{iv.notes}</small>}</div>{iv.status==='proposed'?<div className="pmc-interview-actions"><button disabled={busy} onClick={()=>void answerInterview(iv,'accepted')}>Aceptar</button><button disabled={busy} className="decline" onClick={()=>void answerInterview(iv,'declined')}>No puedo</button></div>:<div><strong>{iv.status==='accepted'?'✓ Entrevista confirmada':iv.status==='declined'?'No aceptada':iv.status==='completed'?'Realizada':'Cancelada'}</strong>{iv.status==='accepted'&&<Link className="pmc-calendar-link" href="/calendario">Ver en calendario</Link>}</div>}</article>)}</div>}
