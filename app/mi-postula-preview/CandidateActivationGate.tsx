...
export default function CandidateActivationGate({jobCount=0}:{jobCount?:number}){
 ...
 async function load(){
  ...
  try{
   ...
   const candidateConsent=Boolean((d?.consents||[]).some((x:any)=>x.consent_type==='candidate_profile_activation'&&x.accepted))
   const candidatePrimary=Boolean(d?.profile?.primary_role==='candidate'&&d?.profile?.onboarding_completed)
   const membership=(d?.memberships||[])[0];setCompanyName(String(membership?.pm_companies?.name||''))
   setState(candidateConsent||meaningful||candidatePrimary?'ready':'needs_activation')
  }catch{setState('needs_activation')}
 }
 useEffect(()=>{void load()},[])
 async function activate(){setBusy(true);setError('');try{const {data}=await cvAuthClient().auth.getSession();const token=data.session?.access_token;if(!token){location.assign('/login');return}const r=await fetch('/api/postula/profile',{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({candidate_settings:true,activate_candidate:true,accept_legal:true,source:'candidate_activation',terms_version:'2026-08-21'})});const d=await r.json().catch(()=>({}));if(!r.ok||!d?.ok)throw new Error(d?.error||'No pudimos activar el perfil.');setState('ready')}catch(e){setError(e instanceof Error?e.message:'No pudimos activar el perfil.')}finally{setBusy(false)}}
