...
import { CV_ACCOUNT_API, SESSION_KEY, cvAuthClient } from '../cv-ia/cvAuth'
...
async function signupTicket(email:string){
  const response=await fetch(SIGNUP_TICKET_API,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email})})
  const data=await response.json().catch(()=>({ok:false,error:'Respuesta inválida'}))
  if(!response.ok||!data?.ok)throw new Error(data?.error||'No pudimos preparar la creación de tu cuenta.')
  return String(data.signup_token||'')
}
...
export default function AccountClient(){
  ...
  async function submit(e:FormEvent){
    ...
    try{
      const client=cvAuthClient(),cleanEmail=email.trim().toLowerCase()
      if(mode==='login'){
        ...
        const ticket=await signupTicket(cleanEmail)
        if(!ticket)throw new Error('No pudimos preparar el registro seguro.')
        const {data,error}=await client.auth.signUp({
          email:cleanEmail,
          password,
          options:{
            emailRedirectTo:'https://postulamejor.com/cuenta',
            data:{app:'postula_mejor',postula_signup_token:ticket},
          },
        })
        if(error)throw error
        if(data.session){
          await finishAccount(data.session.access_token)
          setMessage('Cuenta creada y vinculada.')
        }else{
          setMessage('Cuenta creada. Revisá tu email para confirmar la dirección y después iniciá sesión.')
        }
      }
