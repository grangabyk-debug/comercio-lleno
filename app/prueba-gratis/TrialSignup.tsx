...
type GoogleSession={accessToken:string;refreshToken:string;email:string;fullName:string}
...
function persistGoogleTenant(session:GoogleSession,data:any){
  clearPreviousTenantSession()
  localStorage.setItem('cl_access_token',session.accessToken)
  if(session.refreshToken)localStorage.setItem('cl_refresh_token',session.refreshToken)
...
export default function TrialSignup(){
  ...
  async function refreshChallenge(){
    setChallengeBusy(true);setChallengeAnswer('')
    try{
      const response=await fetch(TRIAL_FUNCTION,{method:'POST',headers:{apikey:PUBLISHABLE_KEY,'Content-Type':'application/json'},body:JSON.stringify({action:'challenge'})})
      const data=await response.json().catch(()=>({}))
      if(!response.ok||!data?.token)throw new Error('No se pudo iniciar la verificación humana.')
      setChallenge({question:String(data.question),token:String(data.token),expires_at:data.expires_at})
  ...
  useEffect(()=>{
    ...
    void supabase.auth.getSession().then(({data,error})=>{
      ...
      if(!auth?.access_token||!auth.user?.email)throw new Error('No se pudo completar el acceso con Google.')
      const name=String(auth.user.user_metadata?.full_name||auth.user.user_metadata?.name||auth.user.email.split('@')[0]||'').trim()
      setGoogleSession({accessToken:auth.access_token,refreshToken:auth.refresh_token||'',email:auth.user.email,fullName:name})
      setFullName(name);setEmail(auth.user.email);setError('')
      window.history.replaceState({},'',window.location.pathname)
    }).catch(e=>setError(e instanceof Error?e.message:String(e))).finally(()=>setGoogleBusy(false))
  },[])
  ...
  function validateCommon(){
    ...
    if(!challenge?.token||!challengeAnswer.trim()){setError('Resolvé la verificación humana para continuar.');return false}
    return true
  }

  async function submitGoogle(){
    setError('')
    if(!googleSession||!validateCommon())return
    setBusy(true)
    try{
      const response=await fetch(TRIAL_FUNCTION,{method:'POST',headers:{apikey:PUBLISHABLE_KEY,Authorization:`Bearer ${googleSession.accessToken}`,'Content-Type':'application/json'},body:JSON.stringify({action:'google',company_name:companyName.trim(),challenge_token:challenge?.token||'',challenge_answer:challengeAnswer,accepted_terms:true,accepted_privacy:true})})
      const data=await response.json().catch(()=>({}))
      if(!response.ok||!data?.ok)throw new Error(data?.error||'No se pudo crear el comercio con Google.')
      persistGoogleTenant(googleSession,data)
      setSuccess(data.existing?'Tu cuenta ya estaba lista. Entrando…':'Listo. Google creó tu acceso y tu comercio ya está preparado.')
      setTimeout(()=>location.replace('/redesign?setup=pending'),350)
    }catch(e){const message=e instanceof Error?e.message:String(e);setError(message);if(/verificación humana/i.test(message))await refreshChallenge()}
    finally{setBusy(false)}
  }

  async function submit(e:FormEvent){
    e.preventDefault();setError('')
    if(googleSession){await submitGoogle();return}
  ...
  async function changeGoogleAccount(){
    ...
    setGoogleSession(null);setFullName('');setEmail('');setCompanyName('');setAccepted(false);setError('');setSuccess('');void refreshChallenge()
  }
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.layout}>
      ...
      <div className={styles.card}>
        <div className={styles.cardTop}><div><span>REGISTRO SIMPLE</span><h2>{googleSession?'Terminá con Google':'Empezá ahora'}</h2><p>{googleSession?'Google ya completó tus datos personales.':'Lo demás lo configurás cuando quieras.'}</p></div><div className={styles.timeBadge}>1 PASO</div></div>

        {!googleSession&&<><button type="button" className={styles.googleButton} onClick={()=>void startGoogle()} disabled={googleBusy||busy}><GoogleMark/><span>{googleBusy?'Abriendo Google…':'Continuar con Google'}</span></button><p className={styles.googleHint}>Google completa tu nombre y email. Después sólo elegís el nombre del comercio.</p><div className={styles.divider}><span>o registrate con email</span></div></>}
        {googleSession&&<div className={styles.googleConnected}><GoogleMark/><div><b>{googleSession.fullName||'Cuenta de Google'}</b><span>{googleSession.email}</span></div><button type="button" onClick={()=>void changeGoogleAccount()}>Cambiar</button></div>}

        <form className={styles.form} onSubmit={submit}>
          {!googleSession&&<div className={styles.fieldGrid}><label className={styles.label}>Tu nombre<input className={styles.input} autoComplete="name" required value={fullName} onChange={e=>setFullName(e.target.value)} placeholder="Nombre y apellido"/></label><label className={styles.label}>Email<input className={styles.input} autoComplete="email" required type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="vos@comercio.com"/></label></div>}
          ...
          {!googleSession&&<label className={styles.label}>Contraseña<div className={styles.passwordWrap}><input className={styles.input} autoComplete="new-password" required type={showPassword?'text':'password'} value={password} onChange={e=>setPassword(e.target.value)} placeholder="Ej: Comercio#2026"/><button type="button" className={styles.eye} onClick={()=>setShowPassword(value=>!value)} aria-label={showPassword?'Ocultar contraseña':'Mostrar contraseña'}><EyeIcon open={showPassword}/></button></div><small className={strong(password)?styles.goodHint:styles.hint}>8+ caracteres, mayúscula, número y signo.</small></label>}
          <div className={styles.challenge}><div><span>VERIFICACIÓN</span><b>{challengeBusy?'Generando…':challenge?.question||'No disponible'}</b></div><input inputMode="numeric" value={challengeAnswer} onChange={e=>setChallengeAnswer(e.target.value.replace(/\D/g,'').slice(0,3))} placeholder="Respuesta" disabled={challengeBusy||!challenge}/><button type="button" onClick={()=>void refreshChallenge()} disabled={challengeBusy}>Otra</button></div>
          ...
          <button className={styles.button} disabled={busy||googleBusy}>{busy?'Creando tu comercio…':googleSession?'Crear mi comercio con Google':'Crear mi comercio'}</button>
