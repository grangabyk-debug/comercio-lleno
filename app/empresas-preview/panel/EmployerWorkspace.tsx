...
const views:readonly [View,string,string][]=[['resumen','Resumen','⌂'],['candidatos','Candidatos','◎'],['mensajes','Mensajes','✉'],['publicaciones','Publicaciones','▤'],['empresa','Empresa y configuración','⚙']]
...
export default function EmployerWorkspace(){
 ...
 return <div className="pm48-workspace" data-view={view}>
  <aside className="pm48-sidebar" aria-label="Panel de empresa"><div className="pm48-brand"><i>PM</i><div><b>Postulá Mejor</b><small>Espacio empresa</small></div></div><nav>{views.map(([key,label,icon])=><button type="button" key={key} data-on={view===key} onClick={()=>choose(key)}><span>{icon}</span>{label}</button>)}</nav><div className="pm48-side-bottom"><a href="/empresas/publicar">＋ Nueva búsqueda</a><a href="/">Ver sitio público ↗</a></div></aside>
  <main className="pm48-workspace-main">
   ...
   <div className="pm48-live"><EmployerDashboardLive/>{view==='candidatos'&&<EmployerCandidateReputationInline/>}</div>
   {view==='publicaciones'&&<section className="pm48-view pm48-publications"><header><div><small>PUBLICACIONES</small><h1>Todo lo que tu empresa tiene activo.</h1><p>Entrá a Empleos o Servicios Flex sólo cuando necesites administrarlos.</p></div><a href="/empresas/publicar">Nueva búsqueda</a></header><div className="pm48-segmented"><button type="button" data-on={publicationView==='empleos'} onClick={()=>setPublicationView('empleos')}>Empleos</button><button type="button" data-on={publicationView==='flex'} onClick={()=>setPublicationView('flex')}>Servicios Flex</button></div><div className="pm48-module">{publicationView==='empleos'?<EmployerJobManager/>:<FlexManager accountMode/>}</div></section>}
   {view==='empresa'&&<section className="pm48-view pm48-settings"><header><div><small>EMPRESA</small><h1>Cuenta y configuración.</h1><p>Datos, identidad y preferencias en el mismo panel.</p></div></header><div className="pm48-settings-stack"><CompanySettings/><CompanyBranding/><details className="pm48-settings-details"><summary><div><small>NOTIFICACIONES</small><b>Avisos y permisos del dispositivo</b></div><span>Administrar</span></summary><div><NotificationSettings audience="employer"/></div></details><details className="pm48-settings-details"><summary><div><small>CONFIGURACIÓN AVANZADA</small><b>Validación, créditos y puesta a punto</b></div><span>Administrar</span></summary><div><EmployerSetupOnboarding/></div></details></div></section>}
  </main>
 </div>
}
