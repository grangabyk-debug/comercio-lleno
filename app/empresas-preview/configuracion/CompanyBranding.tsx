...
type Membership={role?:string;pm_companies?:Company|Company[]|null}
...
export default function CompanyBranding(){
 ...
 async function readCompany(token:string){
  const r=await fetch('/api/postula/company',{headers:{Authorization:`Bearer ${token}`},cache:'no-store'});const d=await r.json().catch(()=>({}))
  const memberships=(d?.memberships||[]) as Membership[]
  const member=memberships.find(m=>['owner','admin'].includes(String(m?.role||'')))||memberships[0]
  const c=firstCompany(member?.pm_companies)
  if(c){setCompany(c);setRole(String(member?.role||''));setLogoFailed(false)}
 ...
 async function upload(file:File){if(!company||!['owner','admin'].includes(role))return;setBusy(true);setNotice('');let uploaded='';const previous=company.logo_path||'';try{if(file.size>2*1024*1024)throw new Error('El logo puede pesar hasta 2 MB.');if(!['image/jpeg','image/png','image/webp'].includes(file.type))throw new Error('Usá JPG, PNG o WEBP.');const client=cvAuthClient();const {data}=await client.auth.getSession();const token=data.session?.access_token;if(!token)throw new Error('Tu sesión venció.');uploaded=`${company.id}/logo-${Date.now()}.${extension(file)}`;const {error}=await client.storage.from('postula-branding').upload(uploaded,file,{upsert:false,contentType:file.type,cacheControl:'3600'});if(error)throw error;const r=await fetch('/api/postula/company',{method:'PATCH',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({company_id:company.id,logo_path:uploaded})});const d=await r.json().catch(()=>({}));if(!r.ok||!d?.ok)throw new Error(d?.error||'No pudimos guardar la imagen de la empresa.');setCompany(v=>v?{...v,logo_path:uploaded}:v);setLogoFailed(false);const saved=await readCompany(token);if(saved?.logo_path!==uploaded)throw new Error('La imagen se subió pero no quedó asociada a la empresa. Probá nuevamente.');if(previous&&previous!==uploaded)await client.storage.from('postula-branding').remove([previous]).catch(()=>{});setNotice('Imagen de empresa guardada. Ya quedó asociada al perfil y a tus publicaciones públicas.')}catch(e){if(uploaded)await cvAuthClient().storage.from('postula-branding').remove([uploaded]).catch(()=>{});if(previous)setCompany(v=>v?{...v,logo_path:previous}:v);setNotice(e instanceof Error?e.message:'No pudimos subir la imagen.')}finally{setBusy(false);if(input.current)input.current.value=''}}
