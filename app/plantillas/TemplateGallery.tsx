...
const newOnes=new Set(['pm-pastel','pm-naranja','pm-producto','pm-remoto','pm-rosario','pm-palermo'])
...
export default function TemplateGallery(){
 ...
 const heroA=cvTemplates.find(t=>t.id==='pm-ejecutivo')||cvTemplates[0]
 const heroB=cvTemplates.find(t=>t.id==='pm-palermo')||cvTemplates[1]
 const heroC=cvTemplates.find(t=>t.id==='pm-rosario')||cvTemplates[2]
 ...
 return <>
  <section className="pmt-hero pmt-hero-v3">
   <div><span className="pmt-kicker">{totalTemplates} DISEÑOS REALES · {freeTemplateCount} GRATIS</span><h1>CVs que dan ganas de usar, no plantillas que parecen de hace diez años.</h1><p>Rediseñamos la biblioteca para que cada modelo tenga una identidad clara. Hay opciones ATS, ejecutivas, creativas, técnicas, comerciales y por rubro. Lo que ves en la vista previa corresponde al estilo del archivo que descargás.</p><div className="pmt-hero-actions"><a className="pmt-primary" href="#modelos">Elegir mi diseño</a><a className="pmt-secondary" href="/mejorar-cv">Mejorar mi CV con IA</a></div><div className="pmt-hero-pills"><span>✓ Word editable</span><span>✓ PDF de muestra</span><span>✓ Compatible con Mejorar CV</span><span>✓ {freeTemplateCount} opciones gratis</span></div></div>
  ...
  <section className="pmt-summary"><div><strong>{totalTemplates}</strong><span>modelos distintos</span></div><div><strong>{freeTemplateCount}</strong><span>gratis para descargar ahora</span></div><div><strong>{proTemplateCount}</strong><span>diseños incluidos con CV Pro+</span></div></section>
  <section className="pmt-library-intro"><div><span>COLECCIÓN 2026</span><h2>Elegí por estilo, no por obligación.</h2></div><p>Las {freeTemplateCount} plantillas gratuitas ya cubren estilos ATS, profesionales, técnicos, creativos, comerciales y de primer empleo. Los Pro+ suman composiciones más avanzadas y estilos por rubro.</p></section>
