...
export default function MobileNavigation({active='inicio'}:{active?:Active}){
 ...
 const sheet=open?<div className="pm46-sheet-backdrop" role="presentation" onMouseDown={event=>{if(event.currentTarget===event.target)setOpen(false)}}><section className="pm46-sheet" role="dialog" aria-modal="true" aria-label="Menú móvil de Postulá Mejor">
  ...
  {guest?<>
   <div className="pm46-auth-actions">{employer?<><Link href="/empresas/login" onClick={close}>Ingresar como empresa</Link><Link href="/empresas/registro" className="secondary" onClick={close}>Crear cuenta empresa</Link></>:<><Link href="/login?next=%2F" onClick={close}>Iniciar sesión</Link><Link href="/registro?next=%2F" className="secondary" onClick={close}>Crear cuenta gratis</Link></>}</div>
