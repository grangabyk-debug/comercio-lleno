...
export const metadata={
 ...
 description:'Empleos, Trabajos Flex, CV, orientación y conversaciones en una experiencia laboral más humana.',
 robots:{index:true,follow:true},
 alternates:{canonical:'https://postulamejor.com/'},
 openGraph:{title:'Postulá Mejor | Trabajo, CV y oportunidades',description:'Empleos, Trabajos Flex, CV, orientación y conversaciones en una experiencia laboral más humana.',url:'https://postulamejor.com/'},
 twitter:{card:'summary',title:'Postulá Mejor | Trabajo, CV y oportunidades',description:'Empleos, Trabajos Flex, CV, orientación y conversaciones en una experiencia laboral más humana.'},
}
...
export default async function PostulaPreview(){
 ...
 return <main className={`${styles.page} pm7-page pm7-candidate`}>
  ...
  <section className="pm7-hero"><div className="pm7-hero-bg one"/><div className="pm7-hero-bg two"/><div className="pm7-hero-inner">
   <div className="pm7-hero-copy"><span className="pm7-eyebrow coral">TRABAJO SIN CARA DE TRÁMITE</span><h1>Encontrá algo<br/>que te dé <em>ganas.</em></h1><p>Empleos, trabajos flexibles, gente, empresas y herramientas para moverte. Más conversación, menos formulario eterno.</p><div className="pm7-search"><div><small>¿Qué querés hacer?</small><b>ventas, café, diseño, logística…</b></div><div><small>¿Dónde?</small><b>CABA y Buenos Aires</b></div><Link href="/empleos">Buscar</Link></div><div className="pm7-hero-actions"><Link href="/empleos" className="pm7-btn-black">Explorar empleos</Link><Link href="/mejorar-cv" className="pm7-btn-glass">Mejorar mi CV</Link><Link href="/trabajos-flex" className="pm7-btn-glass">Ver Trabajos Flex</Link></div><div className="pm7-hero-proof"><span><i/>Postularte es gratis</span><span><i/>Tu perfil es tuyo</span><span><i/>La IA explica, no decide</span></div></div>
  ...
  <section className="pm8-tools"><div className="pm8-tools-head"><div><span className="pm7-eyebrow coral">HERRAMIENTAS GRATIS</span><h2>Tu perfil también se puede entrenar.</h2></div><p>No tenés que comprar nada para empezar. Creá una cuenta simple y usá el diagnóstico ATS, el test vocacional, el creador de primer CV y la biblioteca de plantillas.</p></div><div className="pm8-tool-grid">
   ...
   <Link href="/plantillas" className="pm8-tool"><i/><span>Plantillas</span><h3>Tres diseños gratis. Los Pro se ven antes de pagar.</h3><p>Elegí color, editá y descargá. Pro+ suma más tipografías, estructuras y adaptaciones por puesto.</p><b>Ver biblioteca →</b></Link>
  </div></section>
  ...
  <section className="pm7-feed"><div className="pm7-feed-layout"><div className="pm7-feed-main"><div className="pm7-feed-title"><div><span className="pm7-eyebrow coral">OPORTUNIDADES</span><h2>Un catálogo que se renueva.</h2></div><Link href="/empleos">Ver todas</Link></div><div className="pm7-social-job-list">{featured.map((job,i)=>{const tag=extraTag(job);return <article className="pm7-social-job" key={job.slug}><div className="pm7-social-job-cover" style={{backgroundImage:`url(${jobVisual(job.area,i)})`}}><span>{job.mode}</span><button aria-label="Guardar">♡</button></div><div className="pm7-social-job-body"><div className="pm7-social-company"><span>{initials(job.company)}</span><div><b>{job.company}</b><small>{job.location}</small></div><em>catálogo actualizado</em></div><h3>{job.title}</h3><p>{job.summary}</p><div className="pm7-social-tags"><span>{job.area}</span><span>{job.schedule}</span>{tag&&<span>{tag}</span>}</div><div className="pm7-social-job-foot"><span><i/>{job.external?'Fuente oficial revisada':'Publicada en Postulá Mejor'}</span><Link href={`/empleos/${job.slug}`}>Ver trabajo →</Link></div></div></article>})}</div></div>
