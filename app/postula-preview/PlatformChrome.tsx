  ...
  return <><header className={headerClass}>
    ...
    <nav className={`${styles.nav} pm-social-nav`} aria-label="Navegación Postulá Mejor">
      {audience==='candidate'?<>
        ...
        <Link href="/changas-preview" className="pm-nav-new">Changas <small>NUEVO</small></Link>
        <Link href="/plantillas-preview">CV</Link>
      </>:<>
...
export function MobileNav({active='inicio'}:{active?:'inicio'|'empleos'|'cuenta'|'cv'|'changas'}){
  return <nav className={`${styles.mobileNav} pm-social-mobile-nav`} aria-label="Navegación móvil">
    ...
    <Link href="/changas-preview" data-active={active==='changas'}><i className="pm-nav-i bolt"/>Changas</Link>
    <Link href="/mi-postula-preview" data-active={active==='cuenta'}><i className="pm-nav-i person"/>Perfil</Link>
  </nav>
}

export function PlatformFooter(){return <footer className={`${styles.footer} pm-postula-footer`}><div className={styles.footerInner}><div className="pm-footer-brand"><div className={styles.brand}><span className={styles.mark}>PM</span><span>Postulá Mejor</span></div><p>Trabajo, oportunidades y selección con tecnología explicable. Empleo formal, búsquedas rápidas y changas en una experiencia independiente.</p><div className="pm-footer-trust"><span><i/>Conexión segura HTTPS</span><span><i/>Privacidad y trazabilidad</span><span><i/>IA con decisión humana</span></div></div><div className="pm-footer-links"><Link href="/empleos-preview">Empleos</Link><Link href="/changas-preview">Changas</Link><Link href="/empresas-preview">Empresas</Link><Link href="/plantillas-preview">Plantillas</Link><Link href="/cv-ia">CV Pro+</Link><Link href="/politica-de-privacidad">Privacidad</Link></div></div><div className="pm-footer-legal"><span>© 2026 Postulá Mejor. Todos los derechos reservados.</span><span>Gabriel Alejandro Granvillano · CUIT 20-38422407-6</span><span className="pm-security-seal">SEGURIDAD · DATOS PROTEGIDOS</span></div></footer>}
