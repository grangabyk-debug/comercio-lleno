...
export default function NotificationSettings({audience='candidate'}:{audience?:'candidate'|'employer'}){
 ...
 async function enableBrowser(){
  ...
  try{
   ...
   if(result==='granted'){
    const next={...prefs,applications:true,interviews:true}
    setBrowserEnabled(true);persist(next,true)
    setNotice('Notificaciones activadas en este dispositivo. Mensajes, seguridad y avisos importantes quedan activos por defecto.')
    try{new Notification('Postulá Mejor',{body:'Listo. Este dispositivo puede avisarte sobre mensajes, seguridad y novedades importantes.',tag:'pm-notifications-ready'})}catch{}
 ...
 return <section className="pm16-notifications" aria-labelledby={`pm16-title-${audience}`}>
  <div className="pm16-notif-head"><div><span>NOTIFICACIONES</span><h2 id={`pm16-title-${audience}`}>Enterate de lo importante. Nada más.</h2><p>Elegí cómo querés enterarte de {roleCopy}. Podés recibir avisos tanto en PC como en móvil, según los permisos de cada dispositivo.</p></div><div className="pm16-browser-card" data-on={browserEnabled&&permission==='granted'}><small>ESTE DISPOSITIVO</small><b>{permission==='granted'?(browserEnabled?'Notificaciones activas':'Permiso disponible · avisos pausados'):permission==='denied'?'Notificaciones bloqueadas':permission==='unsupported'?'No compatible':'Notificaciones sin activar'}</b>{permission==='granted'&&browserEnabled?<button type="button" onClick={pauseBrowser}>Pausar en este dispositivo</button>:permission!=='denied'&&permission!=='unsupported'?<button type="button" onClick={enableBrowser}>{permission==='granted'?'Reactivar':'Activar notificaciones'}</button>:<span className="pm16-browser-help">Cambialo desde los permisos del sitio en tu navegador.</span>}</div></div>
