...
const PACKS={flex1:{credits:1,amount:1990,label:'1 publicación Flex'},flex5:{credits:5,amount:7900,label:'5 publicaciones Flex'},flex10:{credits:10,amount:13900,label:'10 publicaciones Flex'}} as const
...
export async function POST(req:NextRequest){
 ...
 const token=process.env.POSTULA_MERCADOPAGO_ACCESS_TOKEN;if(!token)return NextResponse.json({ok:false,code:'billing_not_configured',error:'El cobro online de Trabajo Flex todavía no está habilitado. No se realizó ningún cargo.'},{status:503})
 ...
 const response=await fetch('https://api.mercadopago.com/checkout/preferences',{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({items:[{title:`Postulá Mejor · ${chosen.label}`,quantity:1,currency_id:'ARS',unit_price:chosen.amount}],payer:{email:user.email},external_reference:externalReference,back_urls:{success:`https://postulamejor.com/trabajos-flex?flex_payment=success&purchase=${purchase.id}`,pending:`https://postulamejor.com/trabajos-flex?flex_payment=pending&purchase=${purchase.id}`,failure:`https://postulamejor.com/trabajos-flex?flex_payment=failure&purchase=${purchase.id}`},auto_return:'approved',notification_url:'https://postulamejor.com/api/postula/flex/credits/webhook'})})
