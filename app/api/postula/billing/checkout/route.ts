...
export async function POST(req:NextRequest){
 ...
 if(mode!=='payment'){
  const {data:trial,error:trialError}=await client.rpc('pm_start_company_trial',{p_plan:plan})
  if(trialError)return NextResponse.json({ok:false,error:'No pudimos activar el período gratis ahora. Probá nuevamente.'},{status:500})
  const result=(trial||{}) as Record<string,any>
  if(!result.ok){
   const code=String(result.code||'')
   const status=code==='needs_company'||code==='trial_used'?409:code==='unauthenticated'?401:400
