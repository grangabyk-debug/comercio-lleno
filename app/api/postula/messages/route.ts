...
const conversationSelect='id,application_id,company_id,candidate_user_id,conversation_kind,flex_post_id,flex_response_id,publisher_user_id,last_message_at,pm_companies(name),pm_applications(status,pm_jobs(title)),pm_flex_posts(title,location_text,compensation_text)'
...
async function ensureConversation(db:any,body:any){
 let conversation=clean(body?.conversation_id,80)
...
export async function GET(req:NextRequest){
 ...
 if(id){
  ...
  return NextResponse.json({ok:true,conversation,messages:messages||[],interviews:interviews||[],me:user.id,audience:String(conversation.candidate_user_id)===user.id?'candidate':'employer'})
 }
 ...
 return NextResponse.json({ok:true,conversations:data||[],me:user.id})
}

export async function POST(req:NextRequest){
 const db=client(req);const {data:{user}}=await db.auth.getUser();if(!user)return NextResponse.json({ok:false,error:'Iniciá sesión.'},{status:401})
 const body=await req.json().catch(()=>({}));const action=clean(body?.action,40)||'send'
 let conversation=''
 ...
 if(action==='schedule_interview'){
  ...
  const mode=['presencial','virtual','telefonica','a_coordinar'].includes(String(body?.mode))?String(body.mode):'a_coordinar'
  const duration=Math.max(10,Math.min(480,Number(body?.duration_minutes)||30));const location=clean(body?.location_text,240);const notes=clean(body?.notes,1000)
 ...
 if(action==='respond_interview'){
  ...
  const label=status==='accepted'?'Entrevista confirmada':'Entrevista rechazada';const {data:message}=await db.from('pm_messages').insert({conversation_id:conversation,sender_user_id:user.id,body:label,message_type:'system',metadata:{interview_id:id,status}}).select('id,body,message_type,metadata,created_at,sender_user_id').single()
  if(status==='accepted'&&interview){const {data:conv}=await db.from('pm_conversations').select('application_id').eq('id',conversation).maybeSingle();if(conv?.application_id)await db.from('pm_applications').update({status:'interview',updated_at:new Date().toISOString()}).eq('id',conv.application_id)}
  await db.from('pm_conversations').update({last_message_at:new Date().toISOString()}).eq('id',conversation)
  return NextResponse.json({ok:true,interview,message})
 }
 ...
 const type=body?.message_type==='voice_transcript'?'voice_transcript':'text';const metadata:any={safety_flags:check.flags};if(type==='voice_transcript'){metadata.transcribed=true;metadata.source='employer_voice'}
