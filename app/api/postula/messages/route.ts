...
export async function POST(req:NextRequest){
 ...
 if(action==='respond_interview'){
  ...
  const {data:interview,error:iErr}=await db.rpc('pm_respond_interview',{p_interview:id,p_status:status});if(iErr||!interview||String((interview as any).conversation_id)!==conversation)return NextResponse.json({ok:false,error:iErr?.message||'La propuesta ya fue respondida o no está disponible.'},{status:/forbidden/i.test(iErr?.message||'')?403:400})
