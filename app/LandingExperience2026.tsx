...
const storePhoto='https://images.pexels.com/photos/3184465/pexels-photo-3184465.jpeg?auto=compress&cs=tinysrgb&w=1800'
...
const modules=[
  ['01','Vendé','Cobro simple, caja, descuentos, presupuestos y distintos medios de pago.'],
  ['02','Facturá','Factura electrónica ARCA integrada al mismo flujo de venta.'],
  ['03','Controlá','Stock, precios, categorías, alertas y movimientos en tiempo real.'],
  ['04','Entendé','Reportes y asistente IA para leer el negocio sin planillas eternas.'],
]
...
export default function LandingExperience2026(){return <>
  <section className={styles.story}>
    ...
    <div className={styles.storyCopy}><p>UNA NUEVA FORMA DE MANEJAR TU COMERCIO</p><h2>La tecnología tiene que sentirse <em>simple.</em></h2><span>Comercio Lleno concentra venta, stock, caja, facturación, cobros e inteligencia artificial en una experiencia clara. Abrís el navegador y trabajás. Nada más.</span><div className={styles.storyStats}><div><b>100%</b><small>online</small></div><div><b>Celular</b><small>+ computadora</small></div><div><b>1 sistema</b><small>toda la operación</small></div></div></div>
    <div className={styles.storyVisual}><img src={storePhoto} alt="Persona trabajando con tecnología en un comercio"/><div className={styles.photoOverlay}/><div className={styles.floatTag}><span>● EN VIVO</span><b>Tu negocio, sincronizado.</b><small>Lo que pasa en caja se refleja en todo el sistema.</small></div></div>
  </section>

  <section className={styles.devices}>
    <div className={styles.deviceHeading}><p>EL MISMO NEGOCIO · EN TODOS TUS DISPOSITIVOS</p><h2>De la caja al bolsillo.<br/><em>Sin perder el hilo.</em></h2><span>Entrá desde una notebook en el mostrador o desde el celular caminando por el local. Los datos siguen siendo los mismos.</span></div>
    <div className={styles.deviceStage}><div className={styles.deviceOrbOne}/><div className={styles.deviceOrbTwo}/><LaptopMock/><PhoneMock/><div className={`${styles.deviceChip} ${styles.chipOne}`}><span>ARCA</span><b>Factura integrada</b></div><div className={`${styles.deviceChip} ${styles.chipTwo}`}><span>STOCK</span><b>Actualización en vivo</b></div><div className={`${styles.deviceChip} ${styles.chipThree}`}><span>IA</span><b>Consultas instantáneas</b></div></div>
  </section>

  <section className={styles.flow}>
    <div className={styles.flowTitle}><p>TODO CONECTADO</p><h2>Menos pasos.<br/>Más negocio.</h2></div>
    <div className={styles.flowGrid}>{modules.map(([number,title,text])=><article key={number}><span>{number}</span><div className={styles.flowIcon}/><h3>{title}</h3><p>{text}</p></article>)}</div>
  </section>
