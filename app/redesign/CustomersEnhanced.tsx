...
export default function CustomersEnhanced({data,session,refresh,message}:{data:CommerceSnapshot;session:TenantSession;refresh:()=>Promise<void>;message:(m:string)=>void}) {
  ...
  return <>
    ...
    <div className={`${core.table} ${core.customerTable}`}>
      ...
      {rows.map(c=><div className={core.tableRow} key={c.id}><span><b>{c.name}</b></span><span>{c.phone||'—'}</span><span>{c.email||'—'}</span><span>{c.tax_id||'—'}</span><span style={{display:'flex',gap:8,flexWrap:'wrap'}}><button className={core.secondary} onClick={()=>setEdit({...c})}>Editar</button><button className={core.ghostDanger} disabled={busy} onClick={()=>void remove(c)}>Eliminar</button></span></div>)}
    </div>
