...
const quick=['¿Cómo vienen las ventas hoy?','¿Qué productos tienen stock bajo?','¿Cómo agrego o edito un producto?','¿Cómo vinculo WhatsApp?','¿Cómo conecto Mercado Pago Point?','¿Qué puedo hacer con Comercio Lleno?']
...
function FormattedMessage({content}:{content:string}){
  ...
  return <div className={styles.messageContent}>{lines.map((raw,index)=>{
    const line=raw.trim()
    if(!line)return <span className={styles.messageSpacer} key={index}/>
    const bullet=line.match(/^[-*•]\s+(.+)$/)
    if(bullet)return <div className={styles.messageListRow} key={index}><i>•</i><span><InlineText text={bullet[1]}/></span></div>
    const numbered=line.match(/^(\d+)[.)]\s+(.+)$/)
    if(numbered)return <div className={styles.messageListRow} key={index}><i>{numbered[1]}.</i><span><InlineText text={numbered[2]}/></span></div>
    const heading=line.replace(/^#{1,6}\s+/,'')
    return <div className={styles.messageLine} key={index}><InlineText text={heading}/></div>
  })}</div>
}

export default function UnifiedAssistant(){
  const[messages,setMessages]=useState<Message[]>([{role:'assistant',content:'Hola. Soy el Asistente IA de Comercio Lleno. Puedo consultar datos reales de tu comercio y explicarte cómo usar el sistema, sus integraciones y la versión móvil.'}])
  ...
  return <><div className={styles.head}><div><span>SOPORTE INTELIGENTE</span><h1>Asistente IA</h1><p>Consultá datos reales del comercio y aprendé a usar funciones, integraciones y configuración de Comercio Lleno.</p></div><div className={styles.aiBadge}>✦ IA · acceso protegido</div></div><div className={styles.layout}><section className={styles.chatCard}><div className={styles.chatTitle}><div className={styles.avatar}>✦</div><div><b>Asistente Comercio Lleno</b><small>Sólo usuarios autenticados · datos del tenant actual</small></div></div><div className={styles.messages}>{messages.map((m,i)=><div key={i} className={m.role==='user'?styles.user:styles.bot}>{m.role==='assistant'?<FormattedMessage content={m.content}/>:m.content}</div>)}{busy&&<div className={styles.thinking}><i/><i/><i/></div>}</div><div className={styles.quick}>{quick.map(q=><button type="button" key={q} disabled={busy} onClick={()=>void ask(q)}>{q}</button>)}</div><form className={styles.form} onSubmit={submit}><input value={text} onChange={e=>setText(e.target.value)} placeholder="Ej: ¿Cómo vinculo WhatsApp al comercio?" autoComplete="off"/><button disabled={busy||!text.trim()}>{busy?'Consultando…':'Enviar'}</button></form></section><aside className={styles.side}><div className={styles.aiOrder}><div className={styles.aiOrderHead}><div className={styles.aiOrderSpark}>✦</div><div><span>REPOSICIÓN INTELIGENTE</span><h3>Pedido IA+</h3></div></div><p>Prioridad sugerida según demanda de los últimos 30 días y el stock actual. Pensada como lista rápida para salir a comprar.</p><div className={styles.aiOrderMeta}>✦ Demanda + stock bajo · hasta 10 prioridades</div><div className={styles.aiOrderList}>{priorityBusy?<div className={styles.aiOrderState}>Analizando tu comercio…</div>:priorityError&&!priority.length?<div className={styles.aiOrderState}>{priorityError}</div>:priority.length?priority.map((p,i)=><div className={styles.aiOrderRow} key={p.id||`${p.name}-${i}`}><b>{i+1}</b><span>{p.name}</span></div>):<div className={styles.aiOrderState}>Todavía no hay suficiente historial para sugerir un pedido.</div>}</div>{priority.length>0&&<div style={{display:'flex',gap:7,alignItems:'center',marginTop:10}}><button type="button" onClick={downloadOrder} style={{flex:1,border:'1px solid #d8e2dd',background:'#fff',borderRadius:9,padding:'8px 10px',fontSize:11,fontWeight:900,cursor:'pointer'}}>↓ Descargar</button><button type="button" onClick={shareOrder} title="Reenviar al WhatsApp del propietario" aria-label="Reenviar pedido por WhatsApp" style={{width:34,height:34,border:'1px solid #bfe0cc',background:'#edf9f2',color:'#147449',borderRadius:'50%',fontWeight:950,cursor:'pointer'}}>◉</button></div>}{priorityError&&priority.length>0&&<small style={{display:'block',color:'#a24a4a',fontSize:11,marginTop:6}}>{priorityError}</small>}</div><HumanSupportChat/></aside></div></>
}
