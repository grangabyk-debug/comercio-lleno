...
const STEPS:Step[]=[
  ...
  {id:'impresora',label:'Prepará la impresora y tickets',icon:'▤',primary:'Configuración',secondary:'Impresora y tickets',optional:true},
...
export default function OnboardingGuide(){
  ...
  useEffect(()=>{
    ...
    let localDeferred:Record<string,boolean>={};try{localDeferred=JSON.parse(localStorage.getItem(`cl_setup_deferred_${session.companyId}`)||'{}')}catch{}setDeferred(localDeferred)
  ...
  function saveDeferred(next:Record<string,boolean>){setDeferred(next);if(companyId)localStorage.setItem(`cl_setup_deferred_${companyId}`,JSON.stringify(next))}
  function go(step:Step){
    if(step.id==='arca'&&companyId)sessionStorage.setItem(`cl_onboarding_active_step_${companyId}`,'arca')
    else if(!step.optional)saveProgress({...progress,[step.id]:true})
    const opened=clickButton(step.primary);if(opened&&step.secondary)setTimeout(()=>clickButton(step.secondary!),160)
  }
  ...
  function deferOptional(stepId:string){saveDeferred({...deferred,[stepId]:true})}
  function deferArca(){
    const next={...deferred,arca:true};saveDeferred(next)
    if(companyId){
  ...
  return <aside ref={wrapRef} className={styles.wrap} style={position?{left:position.left,top:position.top,right:'auto'}:undefined} aria-label="Configuración inicial de Comercio Lleno"><div className={styles.card}><div className={styles.head} onPointerDown={startDrag} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag}><div className={styles.headRow}><div className={styles.titleBlock}><b>{incomplete?'⚠ Terminá de configurar ': '👋 Prepará '}{companyName}</b><small>{incomplete?'Faltan datos obligatorios del comercio · podés completarlos ahora':'Tu puesta a punto de Comercio Lleno · impresora y ARCA pueden configurarse después'}</small></div><div className={styles.headActions}><span>{completedEssential} de {ESSENTIAL_STEPS.length}</span><button type="button" onClick={minimize} title="Minimizar guía">—</button></div></div><div className={styles.progress}><i style={{width:`${percent}%`}}/></div></div><div className={styles.body}>
    ...
    {essentialReady?<div className={styles.complete}>✓ Lo esencial ya está listo. La impresora y ARCA quedan como pasos opcionales y podés hacerlos más adelante.</div>:<p>Te acompañamos durante la configuración. Para probar el sistema solo necesitás completar los pasos esenciales; la impresora y ARCA no son obligatorios durante la prueba.</p>}
    {STEPS.map((step,index)=>{
      const done=Boolean(progress[step.id])&&!((step.id==='datos')&&incomplete)
      const postponed=Boolean(deferred[step.id])&&!done
      const row=<button key={`${step.id}-row`} className={`${styles.step} ${done?styles.done:''}`} onClick={()=>go(step)}><i>{done?'✓':step.icon}</i><span>{index+1}. {step.label}{step.optional&&<b style={{display:'inline-block',marginLeft:7,padding:'2px 6px',borderRadius:999,background:'#eef5ff',color:'#2d65a8',fontSize:9,textDecoration:'none'}}>OPCIONAL</b>}</span><small>{done?'Listo':postponed?'Después':'Ir →'}</small></button>
      if(!step.optional||done||postponed)return row
      if(step.id==='impresora')return <div key={step.id}>{row}<div style={{margin:'-1px 0 9px',padding:'10px 11px',border:'1px solid #d8e6f7',borderRadius:10,background:'#f6f9fd',color:'#38536f'}}><b style={{display:'block',fontSize:11,marginBottom:3}}>La impresora no es obligatoria ahora.</b><span style={{display:'block',fontSize:10.5,lineHeight:1.45}}>Podés usar Comercio Lleno durante el período de prueba sin conectar una impresora. Cuando quieras imprimir tickets, la configurás desde Configuración → Impresora y tickets.</span><button type="button" onClick={()=>deferOptional('impresora')} style={{width:'100%',marginTop:9,border:'1px solid #b9cde4',background:'#fff',color:'#254b73',borderRadius:9,padding:'9px 10px',fontSize:10.5,fontWeight:900,cursor:'pointer'}}>Lo hago después</button></div></div>
      return <div key={step.id}>{row}<div style={{margin:'-1px 0 9px',padding:'10px 11px',border:'1px solid #d8e6f7',borderRadius:10,background:'#f6f9fd',color:'#38536f'}}><b style={{display:'block',fontSize:11,marginBottom:3}}>ARCA no es obligatorio ahora.</b><span style={{display:'block',fontSize:10.5,lineHeight:1.45}}>Podés usar el período de prueba, cargar productos y hacer ventas sin configurarlo en este momento. Cuando quieras facturar electrónicamente, lo activás desde Configuración → ARCA.</span><button type="button" onClick={deferArca} style={{width:'100%',marginTop:9,border:'1px solid #b9cde4',background:'#fff',color:'#254b73',borderRadius:9,padding:'9px 10px',fontSize:10.5,fontWeight:900,cursor:'pointer'}}>Lo hago después · cerrar guía</button></div></div>
    })}
