...
import type { ViewKey } from '@/lib/comercio/types'
import styles from './simple-mode.module.css'

type Msg={role:'user'|'assistant';content:string}
function navigate(view:ViewKey){window.dispatchEvent(new CustomEvent<ViewKey>('comercio:navigate-view',{detail:view}))}
...
export default function SimpleModeRuntime(){
  ...
  useEffect(()=>{
    ...
    const key=(e:KeyboardEvent)=>{
      ...
      if(e.key==='Enter'){
        ...
        e.preventDefault();openSection('pos')
        window.setTimeout(()=>{
          const input=Array.from(document.querySelectorAll('input')).find(i=>{const p=i.getAttribute('placeholder')||'';return /producto|código|codigo|escan/i.test(p)}) as HTMLInputElement|undefined
          if(!input)return
  ...
  function openSection(view:ViewKey){setHome(false);window.setTimeout(()=>navigate(view),0)}
  function exit(){setEnabled(false);setHome(true);setAi(false);delete document.documentElement.dataset.simpleMode;window.setTimeout(()=>navigate('dashboard'),0)}
  ...
  return <>
    ...
    {home&&<div className={styles.overlay}><div className={styles.top}><div><span>MODO SIMPLE</span><h1>¿Qué querés hacer?</h1><p>Operación rápida con lector de códigos listo.</p></div><button className={styles.exitButton} onClick={exit}>Salir del Modo Simple</button></div><div className={styles.actions}><button className={styles.sell} onClick={()=>openSection('pos')}><i>$</i><b>Cobrar</b><span>Escanear productos y cobrar</span></button><button className={styles.products} onClick={()=>openSection('products')}><i>▦</i><b>Productos</b><span>Precios y stock</span></button><button className={styles.cash} onClick={()=>openSection('cash')}><i>◷</i><b>Caja diaria</b><span>Abrir, arqueo y cierre</span></button><button className={styles.sales} onClick={()=>openSection('sales')}><i>▤</i><b>Ventas</b><span>Últimas operaciones</span></button></div><div className={styles.scanner}>▣ <b>Scanner activo</b><span>Pasá un código de barras y se abrirá Nueva venta automáticamente.</span></div></div>}
    {!home&&<div className={styles.simpleControls}><button onClick={()=>setHome(true)}>← Inicio simple</button><button className={styles.exitButton} onClick={exit}>Salir del Modo Simple</button></div>}
