...
export default function DashboardEnhanced({ data, todayTotal, todayCount, lowStock, go, canSell, role }: {
  ...
  const previousLabel = previousKey
    ? new Date(`${previousKey}T12:00:00`).toLocaleDateString('es-AR', { day: '2-digit', month: '2-digit' })
    : 'el último día'

  return <>
    <Head eyebrow={data.company.name} title="Inicio" subtitle="Resumen del comercio y accesos de operación.">
      {canSell && <button className={core.primary} onClick={() => go('pos')}>🪙 Nueva venta</button>}
    </Head>
    ...
    <div className={core.kpis}>
      <div className={`${core.kpi} ${core.kpiAccent}`}>
      ...
      <div className={core.kpi}>
        ...
        <Trend current={todayAvg} previous={previousAvg} label={previousLabel} />
      </div>
      <div className={core.kpi}>
        <span>Stock bajo</span>
        <strong>{lowStock}</strong>
        <small>Productos para revisar</small>
      </div>
      <div className={core.kpi}>
        <span>Caja</span>
        <strong>{data.cashRegister?.status === 'open' ? 'Abierta' : 'Cerrada'}</strong>
        <small>{data.cashRegister?.opened_at
          ? `Desde ${new Date(data.cashRegister.opened_at).toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' })}`
          : 'Sin apertura activa'}</small>
      </div>
    </div>

    <div className={core.gridTwo}>
      <div className={core.panel}>
        <div className={core.panelTitle}>
          <div><b>Últimas ventas</b><small>Actividad más reciente</small></div>
        ...
        </div>) : <div className={core.empty}>Todavía no hay ventas.</div>}
      </div>
      ...
      <div className={core.panel}>
        ...
        <div className={core.shortcutGrid}>
          {canSell && <button className={core.shortcut} onClick={() => go('pos')}><span>🪙</span><div><b>Nueva venta</b><small>Scanner, carrito y cobro</small></div></button>}
          <button className={core.shortcut} onClick={() => go('products')}><span>▦</span><div><b>Productos</b><small>Editar precios, stock y costos</small></div></button>
          <button className={core.shortcut} onClick={() => go('cash')}><span>◷</span><div><b>Caja diaria</b><small>Cierre y contador de billetes</small></div></button>
        </div>
      </div>
    </div>
  </>
