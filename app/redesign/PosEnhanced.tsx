...
export default function PosEnhanced({
  ...
  data: CommerceSnapshot; query:string; setQuery:(v:string)=>void; filtered:CommerceSnapshot['products']; cart:CartLine[];
  addProduct:(id:string)=>void; changeQty:(id:string,d:number)=>void; removeProduct:(id:string)=>void;
  subtotal:number; discountKind:'percent'|'amount'; setDiscountKind:(v:'percent'|'amount')=>void; discountValue:number; setDiscountValue:(v:number)=>void;
  discountAmount:number; total:number; customerId:string; setCustomerId:(v:string)=>void; payment:string; setPayment:(v:string)=>void;
  checkout:(mode:CheckoutMode)=>void; busy:boolean; arca:ArcaHealth|null; offline?:boolean; pendingOffline?:number;
}) {
  ...
  const disabled=!cart.length||data.cashRegister?.status!=='open'||busy||total<=0,itemCount=cart.reduce((a,i)=>a+i.qty,0)

  return <>
    <Head eyebrow="PUNTO DE VENTA · F2" title="Nueva venta" subtitle={offline?'Modo offline activo: podés seguir cobrando y sincronizar después.':'Escaneá o buscá un producto. Todo el cobro queda visible a la derecha.'}>
      <div className={core.headBadges}><span className={`${core.badge} ${data.cashRegister?.status==='open'?core.badgeGreen:core.badgeRed}`}>{data.cashRegister?.status==='open'?'● Caja abierta':'● Caja cerrada'}</span><span className={`${core.badge} ${offline?core.badgeAmber:!arcaConfigured?core.badgeAmber:arca?.connected?core.badgeGreen:core.badgeRed}`}>{offline?`● Offline${pendingOffline?` · ${pendingOffline} pend.`:''}`:!arcaConfigured?'● ARCA no configurado':arca?.connected?'● ARCA online':'● ARCA offline'}</span></div>
    </Head>

    <div className={compact.shell}>
      <div className={compact.searchWrap}><div className={core.searchCard}><div className={core.searchBox}><span className={core.searchIcon}><UiIcon name="search" size={19}/></span><input className={core.inputBare} autoFocus value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key==='Enter'&&scan()} placeholder="Escaneá un código o escribí el nombre del producto…"/><button className={core.primary} disabled={!searching} onClick={scan}>Agregar</button></div><div className={core.scanHint}>Scanner USB listo · La lista aparece sólo mientras buscás</div></div>
        {searching&&<div className={compact.results}>{filtered.slice(0,12).map(p=><button key={p.id} className={compact.resultRow} onClick={()=>selectProduct(p.id)}><span className={compact.resultIcon}><UiIcon name="products" size={17}/></span><span className={compact.resultInfo}><b>{p.name}</b><small>{p.barcode||'Sin código'} · {p.category||'General'}</small></span><span className={compact.resultStock}>Stock {p.stock}</span><strong className={compact.resultPrice}>{money.format(p.price)}</strong></button>)}{!filtered.length&&<div style={{padding:18,textAlign:'center',fontSize:10,color:'#74817b'}}>No encontramos productos con esa búsqueda.</div>}</div>}
      </div>
      ...
      <section className={compact.workspace}>
        ...
        <div className={compact.body}>
          <div className={compact.cartPanel}><div className={compact.cartLabel}>PRODUCTOS CARGADOS</div><div className={compact.cart}>
            {cart.length?cart.map(i=><div className={compact.cartLine} key={i.id}><div className={compact.cartProduct}><b>{i.name}</b><small>{money.format(i.price)} c/u</small></div><div className={compact.qty}><button onClick={()=>changeQty(i.id,-1)}>−</button><b>{i.qty}</b><button onClick={()=>changeQty(i.id,1)}>+</button><button className={compact.remove} onClick={()=>removeProduct(i.id)}>×</button></div><strong className={compact.lineTotal}>{money.format(i.price*i.qty)}</strong></div>):<div className={compact.empty}><div><UiIcon name="sale" size={28}/><b>Escaneá o buscá un producto</b><span>Los productos agregados quedan en este panel.</span></div></div>}
          </div></div>

          <div className={compact.controls}>
            {offline?<div className={compact.statusBanner}><b>Venta offline.</b> Si elegís facturar, se enviará cuando vuelva Internet.</div>:!arcaConfigured?<div className={compact.statusBanner}>ARCA no está configurado. Podés cobrar sin facturar.</div>:!arca?.connected?<div className={compact.statusBanner}>ARCA está sin responder. Podés registrar el cobro o dejar la facturación pendiente.</div>:null}
            <div className={compact.tools}><button className={`${compact.tool} ${customerId?compact.toolActive:''}`} onClick={()=>{setShowCustomer(x=>!x);setShowDiscount(false)}}><UiIcon name="user" size={15}/> {customer?customer.name:'Agregar cliente'}</button><button className={`${compact.tool} ${discountAmount>0?compact.toolActive:''}`} onClick={()=>{setShowDiscount(x=>!x);setShowCustomer(false)}}><UiIcon name="discount" size={15}/> {discountAmount>0?`Descuento ${money.format(discountAmount)}`:'Agregar descuento'}</button></div>
            ...
            <span className={compact.paymentLabel}>MEDIO DE PAGO</span><div className={compact.payments}>{payments.map(p=><button key={p} className={`${compact.payment} ${payment===p?compact.paymentSelected:''}`} onClick={()=>setPayment(p)}>{p}</button>)}</div>
