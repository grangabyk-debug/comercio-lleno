...
export default function PosEnhanced({data,query,setQuery,filtered,cart,addProduct,changeQty,removeProduct,subtotal,discountKind,setDiscountKind,discountValue,setDiscountValue,discountAmount,total,customerId,setCustomerId,payment,setPayment,paymentParts,setPaymentParts,checkout,busy,arca,offline=false,pendingOffline=0,createBudget,budgetBusy=false}:{
  ...
  const budgetDisabled=!cart.length||busy||budgetBusy||total<=0
  const budgetButton=<button type="button" disabled={budgetDisabled||!createBudget} onClick={createBudget} title="Crea un presupuesto. No descuenta stock, no mueve caja y no se envía a ARCA." style={{minHeight:42,border:'1px solid #e7aaa5',borderRadius:11,background:budgetDisabled?'#f5eeee':'#fff0ee',color:'#a83e37',fontWeight:950,cursor:budgetDisabled?'not-allowed':'pointer',boxShadow:'inset 0 0 0 1px rgba(255,255,255,.5)'}}>{budgetBusy?'Guardando…':'Presupuesto'}</button>
    ...
    <section className={compact.workspace}>
      ...
      <div className={compact.body}>
        ...
        <div className={compact.controls}>
          ...
          {!splitActive?<div className={compact.payments}>{payments.map(p=><button key={p} className={`${compact.payment} ${payment===p?compact.paymentSelected:''}`} onClick={()=>chooseSingle(p)}>{p}</button>)}{budgetButton}</div>:<><div className={compact.splitPanel}>
          ...
          </div><div style={{marginTop:8}}>{budgetButton}</div></>}
          <div style={{fontSize:10,lineHeight:1.45,color:'#8f4a45',padding:'0 2px'}}>Presupuesto es un documento informativo: no factura, no descuenta stock y no mueve la caja.</div>
