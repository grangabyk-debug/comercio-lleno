...
function ManageBranch({session,branch,staff,assignments,close,saved}:{session:NonNullable<ReturnType<typeof readTenantSession>>;branch:BranchAdmin;staff:StaffProfile[];assignments:BranchAssignment[];close:()=>void;saved:()=>Promise<void>}){
  ...
  return <div className={styles.modal} onMouseDown={e=>e.currentTarget===e.target&&close()}><div className={`${styles.modalCard} ${styles.wide}`}><header><div><span>CONFIGURAR SUCURSAL</span><h3>{branch.name}</h3></div><button type="button" onClick={close}>×</button></header>
