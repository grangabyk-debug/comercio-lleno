 ...
 useEffect(()=>{
  const apply=()=>{
   ...
   while((node=walker.nextNode())){
    ...
    const next=node.nodeValue.replace(/CV Pro/g,'CV PRO').replace(/\bPro\b/g,'PRO')
   ...
   if(filterSection)filterSection.classList.add('postulaTripleFilterSection')
   ...
   if(comparison){
    ...
    if(cols?.[0])cols[0].textContent='Prompt genérico IA'
    if(cols?.[1])cols[1].textContent='PostulaMejor.com'
    if(!comparison.querySelector('[data-ats-comparison]')){
      const sample=comparison.querySelector<HTMLElement>('[class*="compareRow"]')
      if(sample){
        const row=document.createElement('div');row.className=sample.className;row.setAttribute('data-ats-comparison','1')
