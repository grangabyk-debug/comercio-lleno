 ...
 useEffect(()=>{
  ...
  const apply=()=>{
   ...
   if(result&&!result.querySelector('[data-ats-offer]')){
    ...
    if(upsell){
     ...
     button?.addEventListener('click',()=>{
      ...
      const proButton=Array.from(result.querySelectorAll('button')).find(b=>/CREAR MI CV PRO\+?/i.test(b.textContent||'')) as HTMLButtonElement|undefined
   ...
   document.querySelectorAll('[class*="plan"]').forEach(card=>{
    ...
    if((textIs(title,'CV Pro+')||textIs(title,'CV PRO')||textIs(title,'CV Pro'))&&!card.querySelector('[data-ats-included]')){
