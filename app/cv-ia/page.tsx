...
const filterScript=`(()=>{if(window.__pmCvFilters)return;window.__pmCvFilters=true;const decorate=()=>{const h=[...document.querySelectorAll('h2')].find(x=>(x.textContent||'').includes('Tu CV visto desde tres lugares distintos'));const s=h&&h.closest('section');if(!s)return;s.classList.add('pmcv-modern-filter-section');[...s.querySelectorAll('article')].slice(0,3).forEach((c,i)=>{c.classList.add('pmcv-modern-filter-card','pmcv-modern-filter-'+(i+1));const b=c.querySelector('b');if(b)b.textContent=['Filtro ATS','Reclutador','Responsable de área'][i]})};const start=()=>{decorate();new MutationObserver(decorate).observe(document.body,{childList:true,subtree:true})};document.readyState==='loading'?document.addEventListener('DOMContentLoaded',start,{once:true}):start()})()`
