...
import QuickApplyBasicsGuard from './QuickApplyBasicsGuard'
...
export default async function ApplyPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <main className={`${styles.page} pm-candidate-inner-v5 pm7-page`}><PlatformHeader/><div className={styles.wizard}><div className={styles.wizardGrid} data-pm-apply-flow><aside className={styles.steps} aria-label="Pasos de la postulación"><div className={`${styles.step} ${styles.stepActive}`} data-pm-apply-step="1" aria-current="step">1 · Datos y CV</div><div className={styles.step} data-pm-apply-step="2">2 · Presentación</div><div className={styles.step} data-pm-apply-step="3">3 · Consentimiento</div><div className={styles.step} data-pm-apply-step="4">4 · Enviar postulación</div></aside><ApplicationStepSync activeClass={styles.stepActive}/><QuickApplyBasicsGuard/><ApplicationWizard job={job}/></div></div><PlatformFooter/><MobileNav active="empleos"/></main>
}
