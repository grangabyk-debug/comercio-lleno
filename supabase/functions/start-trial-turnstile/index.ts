...
Deno.serve(async(req:Request)=>{
  ...
  let userId='';
  try{
    const userResponse=await fetch(`${SUPABASE_URL}/auth/v1/admin/users`,{method:'POST',headers:headers(),body:JSON.stringify({email,password,email_confirm:true,user_metadata:{full_name:fullName,company_name:companyName,signup_source:'turnstile_90_day_trial'}})});const user=await userResponse.json().catch(()=>({}));
