...
async function adminRest(path: string, init: RequestInit = {}) {
  return fetch(`${SUPABASE_URL}/rest/v1/${path}`, { ...init, headers: adminHeaders(init.headers as Record<string,string> || {}) });
}
...
async function validSignature(req: Request, dataId: string) {
  ...
  const received = fromHex(v1);
  if (!ts || !received || !requestId || !dataId) return false;
  const safeId = dataId.toLowerCase();
  const manifest = `id:${safeId};request-id:${requestId};ts:${ts};`;
...
async function alreadyProcessed(requestId: string) {
  if (!requestId) return false;
  const response = await adminRest(`mercadopago_webhook_events?request_id=eq.${encodeURIComponent(requestId)}&select=request_id&limit=1`);
...
async function markProcessed(requestId: string, dataId: string, eventType: string) {
  if (!requestId) return;
  await adminRest('mercadopago_webhook_events?on_conflict=request_id', {
    method:'POST',
    headers:{ Prefer:'resolution=ignore-duplicates,return=minimal' },
    body:JSON.stringify({ request_id:requestId, data_id:dataId, event_type:eventType, processed_at:new Date().toISOString() }),
  });
...
async function currentSubscription(companyId: string) {
  const response = await adminRest(`company_subscriptions?company_id=eq.${encodeURIComponent(companyId)}&select=status,trial_ends_at,payment_method_added_at&limit=1`);
  const rows = await response.json().catch(() => []);
...
async function syncPreapproval(remote: any, dataId: string, paymentApproved = false) {
  const companyId = String(remote?.external_reference || '');
  ...
  const patch: Record<string,unknown> = {
    ...
    provider_checkout_url:remote?.init_point || null,
    provider_last_synced_at:new Date().toISOString(),
    status:localStatusFromRemote(providerStatus, String(current.status || 'trialing'), current.trial_ends_at),
  ...
  const response = await adminRest(`company_subscriptions?company_id=eq.${encodeURIComponent(companyId)}`, {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    if (type === 'subscription_preapproval' || /preapproval/i.test(String(body?.action || ''))) {
      const remote = await mp(`/preapproval/${encodeURIComponent(dataId)}`);
      const result = await syncPreapproval(remote, dataId, false);
      await markProcessed(requestId, dataId, type);
      return Response.json({ ok:true, ...result }, { status:200 });
    }

    if (type === 'subscription_authorized_payment') {
      const payment = await mp(`/authorized_payments/${encodeURIComponent(dataId)}`);
      const subscriptionId = String(payment?.preapproval_id || payment?.subscription_id || '');
      if (!subscriptionId) {
        await markProcessed(requestId, dataId, type);
      ...
      const result = await syncPreapproval(remote, subscriptionId, paymentApproved);
      await markProcessed(requestId, dataId, type);
      return Response.json({ ok:true, ...result }, { status:200 });
    }
    ...
    return Response.json({ ok:false, error:'No se pudo sincronizar la notificación' }, { status:500 });
  }
});
