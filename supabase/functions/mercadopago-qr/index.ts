...
function cors(req:Request){
  ...
  const ok=o==='https://comerciolleno.com'||o==='https://www.comerciolleno.com'||/^https:\/\/[-a-z0-9]+\.vercel\.app$/i.test(o);
  return {'Access-Control-Allow-Origin':ok?o:'https://comerciolleno.com','Access-Control-Allow-Headers':'authorization,apikey,content-type','Access-Control-Allow-Methods':'POST,OPTIONS','Vary':'Origin'};
}
...
async function mp(path:string,t:string,init:RequestInit={}){const r=await fetch(`${MP}${path}`,{...init,headers:{Authorization:`Bearer ${t}`,'Content-Type':'application/json',...(init.headers||{})},cache:'no-store'}),text=await r.text();let d:any={};try{d=text?JSON.parse(text):{}}catch{d={message:text}}if(!r.ok){const e=new Error(String(d?.message||d?.error||d?.cause?.[0]?.description||`Mercado Pago respondió ${r.status}`)) as Error&{status?:number};e.status=r.status;throw e}return d}
function normalize(o:any){const p=Array.isArray(o?.transactions?.payments)?o.transactions.payments[0]||{}:{},status=String(o?.status||'unknown');return{id:String(o?.id||''),status,status_detail:String(o?.status_detail||p?.status_detail||''),amount:Number(p?.paid_amount||p?.amount||o?.total_amount||0),approved:status==='processed'||String(o?.status_detail||'')==='accredited'||String(p?.status||'')==='processed',final:FINAL.has(status),qr_data:''}}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(!c.terminal_id||String(c.terminal_mode||'')!=='PDV')return json(req,{ok:false,error:'Configurá el Point en modo PDV antes de cobrar con QR.'},409);
    ...
    if(action==='cancel_order'){const id=String(body.order_id||'');if(!id)return json(req,{ok:false,error:'Falta la order.'},400);const current=normalize(await mp(`/v1/orders/${encodeURIComponent(id)}`,t));if(current.final)return json(req,{ok:true,order:current});if(current.status==='at_terminal')return json(req,{ok:false,order:current,error:'La orden ya está en el Point. Cancelala desde la terminal.'},409);const canceled=await mp(`/v1/orders/${encodeURIComponent(id)}/cancel`,t,{method:'POST',headers:{'X-Idempotency-Key':crypto.randomUUID()}});return json(req,{ok:true,order:normalize(canceled)})}
    const amount=Math.round(Number(body.amount||0)*100)/100;if(!(amount>0))return json(req,{ok:false,error:'Importe inválido.'},400);
    const rawSaleId=String(body.sale_id||crypto.randomUUID()),cleanSaleId=rawSaleId.replace(/[^a-zA-Z0-9_-]/g,'').slice(0,36)||crypto.randomUUID().replace(/-/g,''),ref=`CLQR-${a.companyId.slice(0,8)}-${cleanSaleId}`.slice(0,64),ticket=cleanSaleId.replace(/[^a-zA-Z0-9]/g,'').slice(0,20)||crypto.randomUUID().slice(0,8),idempotency=/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(rawSaleId)?rawSaleId:crypto.randomUUID(),formatted=amount.toFixed(2);
    const order=await mp('/v1/orders',t,{method:'POST',headers:{'X-Idempotency-Key':idempotency},body:JSON.stringify({type:'point',external_reference:ref,expiration_time:'PT5M',transactions:{payments:[{amount:formatted}]},config:{point:{terminal_id:String(c.terminal_id),print_on_terminal:'no_ticket',ticket_number:ticket},payment_method:{default_type:'qr'}},description:'Venta Comercio Lleno - QR'})});
    return json(req,{ok:true,order:normalize(order)});
  }catch(e){const status=Number((e as any)?.status||0);return json(req,{ok:false,error:e instanceof Error?e.message:String(e),provider_status:status||null},status>=400&&status<500?status:502)}
});
