...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(action==='cancel_order'){const id=String(body.order_id||'');if(!id)return json(req,{ok:false,error:'Falta la order.'},400);const current=normalize(await mp(`/v1/orders/${encodeURIComponent(id)}`,t));if(current.final)return json(req,{ok:true,order:current});const headers:Record<string,string>={'X-Idempotency-Key':crypto.randomUUID()};if(current.status==='at_terminal')headers['x-allow-cancelable-status']='at_terminal';const canceled=await mp(`/v1/orders/${encodeURIComponent(id)}/cancel`,t,{method:'POST',headers});return json(req,{ok:true,order:normalize(canceled)})}
    ...
    const order=await mp('/v1/orders',t,{method:'POST',headers:{'X-Idempotency-Key':idempotency},body:JSON.stringify({type:'point',external_reference:ref,expiration_time:'PT5M',transactions:{payments:[{amount:formatted}]},config:{point:{terminal_id:String(c.terminal_id),print_on_terminal:'no_ticket',ticket_number:ticket},payment_method:{default_type:'qr'}},description:'Venta Comercio Lleno - cobro QR en Point'})});
    return json(req,{ok:true,external_pos_id:String(c.terminal_pos_id||''),terminal_id:String(c.terminal_id),order:normalize(order)});
  }catch(e){const status=Number((e as any)?.status||0),data=(e as any)?.data||null,providerCode=String(data?.error||data?.code||data?.cause?.[0]?.code||'');console.error('[MP POINT QR]',status,providerCode,e instanceof Error?e.message:String(e));return json(req,{ok:false,error:e instanceof Error?e.message:String(e),provider_status:status||null,provider_error:providerCode||null},status>=400&&status<500?status:502)}
});
