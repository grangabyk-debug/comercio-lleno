...
async function consumeAttempt(req:Request,email:string,userId:string){
  const ipHash=await hmac(`ip:${clientAddress(req)}`);const emailHash=await hmac(`email:${email}`);const challengeHash=await hmac(`google:${userId}:${crypto.randomUUID()}`);
  ...
  const [hourResp,dayResp]=await Promise.all([
    rest(`trial_signup_attempts?ip_hash=eq.${encodeURIComponent(ipHash)}&created_at=gte.${encodeURIComponent(hour)}&select=id&limit=${HOURLY_LIMIT}`),
    rest(`trial_signup_attempts?ip_hash=eq.${encodeURIComponent(ipHash)}&created_at=gte.${encodeURIComponent(day)}&select=id&limit=${DAILY_LIMIT}`)
  ]);
  ...
  const rows=await insert.json().catch(()=>[]);if(!insert.ok||!Array.isArray(rows)||!rows[0]?.id)throw new Error('No se pudo registrar el intento de alta.');return String(rows[0].id)
}
...
async function authenticatedUser(req:Request){const auth=req.headers.get('authorization')||'';if(!auth.toLowerCase().startsWith('bearer '))return null;const token=auth.slice(7).trim();if(!token)return null;const response=await fetch(`${SUPABASE_URL}/auth/v1/user`,{headers:{apikey:SERVICE_ROLE,Authorization:`Bearer ${token}`}});const user=await response.json().catch(()=>({}));return response.ok&&user?.id?user:null}
...
Deno.serve(async(req:Request)=>{
  ...
  if(body.accepted_terms!==true||body.accepted_privacy!==true)return json(req,{ok:false,error:'Para registrarte tenés que aceptar los Términos y Condiciones y la Política de Privacidad.'},400);
  ...
  let attemptId='';try{attemptId=await consumeAttempt(req,email,String(user.id))}catch(e){return json(req,{ok:false,error:e instanceof Error?e.message:String(e)},Number((e as any)?.status)||500)}
