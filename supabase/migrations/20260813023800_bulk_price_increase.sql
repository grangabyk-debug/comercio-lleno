...
begin
  ...
  v_allowed := v_role = 'owner' or coalesce((v_permissions->>'can_edit_products')::boolean, coalesce((v_permissions->>'can_manage_stock')::boolean, true));
...
grant execute on function public.bulk_increase_product_prices(text,numeric) to authenticated, service_role;
